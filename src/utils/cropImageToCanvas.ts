...
export const flipImageToUrl = (image: HTMLImageElement, type: EImageType, flip: number[]) => {
  const cvs = document.createElement("canvas");
  let ctx = cvs.getContext("2d");
  if (!ctx) return "";
  const { width: iWidth, height: iHeight } = image;
...
export const rotateImageToUrl = (image: HTMLImageElement, type: EImageType, rotate: number) => {
  const cvs = document.createElement("canvas");
  let ctx = cvs.getContext("2d");
  if (!ctx) return "";
  const { width: iWidth2, height: iHeight2 } = image;
  [cvs.width, cvs.height] = [iWidth2, iHeight2];
  switch (rotate) {
    case 90:
