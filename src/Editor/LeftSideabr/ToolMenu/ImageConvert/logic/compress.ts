 ...
 * 		})
 *
 * TODO: Blob or dataUrl
 * @returns {Promise(Blob)}
 */
export async function compressAccurately(file: Blob, config: ICompressConfig = {}): Promise<Blob> {
  // 如果指定体积大于原文件体积，则不做处理；
  if (!config.size || config.size * 1024 > file.size) {
    console.log(`指定体积${config.size}大于原文件体积`, file);
    return file;
  }
  ...
  console.log("compressDataURL", compressDataURL);
  ...
  if (compressedSize > file.size) {
    console.log(`压缩后体积${compressedSize}大于原文件体积${file.size}`);
    return file;
  }
